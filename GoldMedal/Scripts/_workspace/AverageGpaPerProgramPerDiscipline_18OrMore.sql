...
SELECT PIDM,  SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE, SHRTCKN_SUBJ_CODE, ROUND( AVG(GradePoints) , 2) AS GPA, COUNT(PIDM) AS NumCourses FROM 
(
	...
	(
		...
		HAVING COUNT(PIDM) > 18 -- filter by a minimum requirement of completed courses
	)
	GROUP BY SHRTCKG_PIDM, SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE, SHRTCKN_SUBJ_CODE, CONCAT(SHRTCKN_SUBJ_CODE, SHRTCKN_CRSE_NUMB)
) pointsTable
GROUP BY PIDM, SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE, SHRTCKN_SUBJ_CODE
HAVING COUNT(PIDM) > 4 -- filter by a minimum requirement of completed courses for that discipline
AND AVG(GradePoints) > 3 -- filter by a threshold GPA
ORDER BY GPA desc
;