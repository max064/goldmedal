...
DEFINE ProgramLevelFilter = 'DG';
  ...
  SELECT SPRIDEN_LAST_NAME || ', ' || SPRIDEN_FIRST_NAME AS STUDENT_NAME, SPRIDEN_ID, '-' AS GRADUATION_DATE, SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE, ROUND( AVG(GradePoints) , 2) AS GPA, COUNT(PIDM) AS NUMBER_OF_COURSES 
  FROM (
  ...
  GROUP BY SPRIDEN_LAST_NAME || ', ' || SPRIDEN_FIRST_NAME, SPRIDEN_ID, '-', SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE
...
UNION ALL 
  ...
  SELECT SPRIDEN_LAST_NAME || ', ' || SPRIDEN_FIRST_NAME AS STUDENT_NAME, SPRIDEN_ID, TO_CHAR(SHRDGMR_GRAD_DATE, 'DD-MON-YYYY'), SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE, ROUND( AVG(GradePoints) , 2) AS GPA, COUNT(PIDM) AS NUMBER_OF_COURSES 
  FROM 
  (
  ...
  WHERE SPRIDEN_PIDM = PIDM
  AND SPRIDEN_CHANGE_IND IS NULL -- to remove repeats due to name changes
  GROUP BY SPRIDEN_LAST_NAME || ', ' || SPRIDEN_FIRST_NAME, SPRIDEN_ID, SHRDGMR_GRAD_DATE, SHRDGMR_SEQ_NO, SHRDGMR_DEGC_CODE
